...
4. **下载所有人格卡** - 下载全部人格卡片
5. **特殊题目分支** - 根据回答动态调整题目
6. **🦞 龙虾Skill** - OpenClaw 龙虾自我人格测试，基于历史数据自动分析

## 🦞 龙虾自我人格测试 Skill

本项目包含一个特殊的 **OpenClaw Skill** - 让龙虾基于自身历史行为数据进行 SBTI 人格测试！

### Skill 功能

- **安装即测**: Skill 安装后自动执行人格测试
- **自动分析**: 基于龙虾历史数据自动计算人格（覆盖 5 大维度、10+ 数据源）
...
```bash
# 进入 Skill 目录
cd skill/lobster-sbti

# 安装后自动执行测试（导出报告+生成卡片）
# 或手动执行：
node scripts/test.mjs --export --card

# 显示详细分析
node scripts/test.mjs --verbose
```
...
Skill 包含完整的 27 种人格类型（25种标准 + 2种特殊）：
...
| **DRUNK** | 酒鬼 | 酒中仙 | 烈酒烧喉，不得不醉。 |

> **HHHH** 和 **DRUNK** 为特殊人格，分别在思维回路清奇和深夜时段触发。

### 输出文件

测试结果保存在 `~/.openclaw/lobster-sbti/output/`：
...
更多详情查看 [`skill/lobster-sbti/SKILL.md`](skill/lobster-sbti/SKILL.md)
